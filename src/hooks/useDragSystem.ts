...
export function useDragSystem({
  ...
  useEffect(() => {
    ...
    // 2. MOVE: If currently holding an item, follow pointer position
    if (activeDragId) {
      setCurrentDragPos((prev) => {
        if (prev && prev.x === pointer.x && prev.y === pointer.y) return prev;
  ...
  }, [pointer, activeDragId, items, handleDropValidation]);
  ...
  const handleMouseDownItem = (e: React.MouseEvent, itemId: string) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      setCurrentDragPos({ x: moveEvent.clientX, y: moveEvent.clientY });
    };
