...
export const HandControlProvider: React.FC<ProviderProps> = ({ children, settings }) => {
  ...
  // Mouse fallback tracking when in mouse mode
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isLeft) {
        setPointer1((prev) => ({ ...prev, ...pState, handIndex: 0, teamId: 'teamA' }));
      } else {
        setPointer2((prev) => ({ ...prev, ...pState, handIndex: 1, teamId: 'teamB' }));
      }
      setDetectedHandsCount(1);
    };
    ...
    const handleMouseDown = (e: MouseEvent) => {
      if (e.button === 0) {
        ...
        if (isLeft) {
          setPointer1((prev) => ({ ...prev, isGrabbing: true, gesture: 'fist' }));
        } else {
          setPointer2((prev) => ({ ...prev, isGrabbing: true, gesture: 'fist' }));
        }
      }
    };
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (e.button === 0) {
        const isLeft = e.clientX / window.innerWidth < 0.5;
        if (isLeft) {
          setPointer1((prev) => ({ ...prev, isGrabbing: false, gesture: 'thumbs_up' }));
          setTimeout(() => {
            setPointer1((prev) => ({ ...prev, gesture: 'open' }));
          }, 350);
        } else {
          setPointer2((prev) => ({ ...prev, isGrabbing: false, gesture: 'thumbs_up' }));
          setTimeout(() => {
            setPointer2((prev) => ({ ...prev, gesture: 'open' }));
          }, 350);
        }
      }
    };
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
        ...
        if (parsedHands.length === 1) {
          ...
          setPointer1((prev) => {
            if (
              ...
              Math.abs(prev.x - targetX1) < 2 &&
              Math.abs(prev.y - targetY1) < 2
            ) {
              return prev;
            }
            return {
          ...
          if (h.rawNormX >= 0.5) {
            ...
            setPointer2((prev) => {
              if (
                ...
                prev.gesture === gesture1 &&
                prev.isGrabbing === isGrabbing1 &&
                Math.abs(prev.x - targetX2) < 2 &&
                Math.abs(prev.y - targetY2) < 2
              ) {
                return prev;
              }
              return {
      ...
      // Frame dropping guard & ~35 FPS cap to prevent frame queuing and CPU thermal throttling
      const camera = new window.Camera(videoRef.current, {
        onFrame: async () => {
          ...
          if (now - lastInferenceTimeRef.current < 28) return; // Cap at ~35 FPS
